...
class BatchLoader(Dataset):
    ...
    def loadLabel(self, labelName):
        ...
        if not (self.imHeight is None or self.imWidth is None):
            ...
            labelIndex = cv2.resize(
                labelIndex, (ncols, nrows), interpolation=cv2.INTER_NEAREST)
        ...
        xIndex, yIndex, labelIndex = \
            xIndex.astype(np.int32), yIndex.astype(
                np.int32), labelIndex.astype(np.int32)
